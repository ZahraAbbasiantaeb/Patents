...
def parse_V1_USPTO(patent):
    
    bs = BeautifulSoup(patent, 'xml')
    patent_id=bs.find('doc-number').string

    patent_title=''
    if bs.find('invention-title'):
        patent_title = bs.find('invention-title').string
    
    patent_desc=''
    all_desc=bs.find_all('description')
    ...
    for tmp in all_desc:
        ...
        to_delete=tmp.find_all('figref')
        for elem in to_delete:
            elem.decompose()
            
        to_delete=tmp.find_all('img')
        for elem in to_delete:
            print("image")
            elem.decompose()
...
def read_all_patents(mypath):
    ...
    for path in file_list:
        all_patents.append(get_patents_from_xml(path))
    print(perf_counter()-curr)
...
def get_patents_from_xml(folder_path):
    ...
    patents = patents[1:]
    patents = [xml_header+patent for patent in patents]
...
for year in years:
    ...
    utility_patents = get_utility_patents_2(all_patents)
    ...
    for key in utility_patents:
        ...
        for patent in utility_patents[key]:
            patent_number, patent_text = parse_V1_USPTO(patent)
