...
def modify_patent(pat):
    
    pat=pat.replace('</B110>', '</dnum>')
    pat=pat.replace('<B110>', '<dnum>')

    pat=pat.replace('</B130>', '</dkind>')
    pat=pat.replace('<B130>', '<dkind>')

    pat=pat.replace('</B140>', '</ddate>')
    pat=pat.replace('<B140>', '<ddate>')

    pat=pat.replace('</B540>', '</dttl>')
    pat=pat.replace('<B540>', '<dttl>')

    pat=pat.replace('</B570>', '</dabs>')
    pat=pat.replace('<B570>', '<dabs>')
    pat=pat.replace('</B570>', '</dabs>')
    
    pat=pat.replace('<PDAT', '<pdat')
    pat=pat.replace('</PDAT>', '</pdat>') 
    
    pat=pat.replace('<STEXT>', '<stext>')
    pat=pat.replace('</STEXT>', '</stext>')
    
    pat=pat.replace('<PARA', '<para')
    pat=pat.replace('</PARA>', '</para>')
    
    pat=pat.replace('<BOLD', '<bold')
    pat=pat.replace('</BOLD>', '</bold>') 
    
    pat=pat.replace('<HIL', '<hil')
    pat=pat.replace('</HIL>', '</hil>') 
    ...
    pat=pat.replace('<BTEXT>', '<btext>')
    pat=pat.replace('</BTEXT>', '</btext>') 

    pat=pat.replace('<DETDESC>', '<detdesc>')
    pat=pat.replace('</DETDESC>', '</detdesc>') 
    
    pat=pat.replace('<SDOCL>', '<sdocl')
    pat=pat.replace('</SDOCL>', '</sdocl>') 
    
    pat=pat.replace('<DRWDESC>', '<drwdesc>')
    pat=pat.replace('</DRWDESC>', '</drwdesc>') 
    
    pat=pat.replace('<BRFSUM>', '<brfsum>')
    pat=pat.replace('</BRFSUM>', '</brfsum>') 

    pat=pat.replace('<SDODE>', '<sdode>')
    pat=pat.replace('</SDODE>', '</sdode>') 
    
    pat=pat.replace('<SDOBI>', '<sdobi>')
    pat=pat.replace('</SDOBI>', '</sdobi>') 
    
    pat=pat.replace('<SDOAB>', '<sdoab>')
    pat=pat.replace('</SDOAB>', '</sdoab>') 
    
    return pat


def parse_V2_USPTO(patent):

    patent = modify_patent(patent)
    bs = BeautifulSoup(patent, 'xml')
    
    patent_id = bs.find('dnum').string

    patent_title = bs.find('dttl').string

    patent_desc = ''
    all_desc = bs.find_all('detdesc')
    
    for tmp in all_desc:
        to_delete = tmp.find_all('maths')
        for elem in to_delete:
            elem.decompose()
        
        to_delete = tmp.find_all('table')
        for elem in to_delete:
            elem.decompose()
        
        to_delete = tmp.find_all('chemistry')
        for elem in to_delete:
            elem.decompose()
 
        to_delete = tmp.find_all('MATH-US')
        for elem in to_delete:
            elem.decompose()
            
        patent_desc += tmp.text

    if (patent_title!=None and len(patent_desc)>0):
...
def read_all_patents_with_pool(mypath):
    
    pool = Pool(32)
    file_list = [mypath+f for f in listdir(mypath) if isfile(join(mypath, f))]
    curr = perf_counter()
    all_patents = pool.map(get_patents_from_xml, file_list)
    print(perf_counter()-curr)
...
def read_all_patents(mypath):
    ...
    for path in file_list:
        all_patents+=get_patents_from_xml(path)
    print(perf_counter()-curr)
    
    return all_patents

def get_patents_from_xml(folder_path):
    with open(folder_path, encoding='utf8', errors='ignore') as f:
        data = f.read()
    patents=data.split(xml_header)
    patents=patents[1:]
    patents=[(xml_header+patent).replace("&"," ") for patent in patents]
    
    return patents


def get_utility_patents_2(all_patents):
    ...
    for patents in all_patents:
        ...
        for patent in patents:
            xml_as_bytes = str.encode(patent)
            tree = etree_lxml.fromstring(xml_as_bytes)
            patent_id = tree.find('.//publication-reference/document-id/doc-number').text
            
            if patent_id.isdigit() and len(patent_id) in [6, 7, 8]:
                if not (patent_id in utility_patents):
                    utility_patents[patent_id] = [patent]
                else:
                    utility_patents[patent_id].append(patent)
                    

    print(timer_start-perf_counter())

    return utility_patents


def get_utility_patents(patents):
    ...
    for patent in patents:
        xml_as_bytes = str.encode(patent)
        parser = etree_lxml.XMLParser(ns_clean=True, huge_tree=True, recover=True)
        tree = etree_lxml.fromstring(xml_as_bytes, parser)
        patent_id=tree.find('.//B110/DNUM/PDAT').text
...
def insert_results_to_sql(patent_id, reference, fields_vec, match_score, exact_match, matched_entry):
    ...
    while not write_bool:
        try:
            ...
            time.sleep(0.01)
            line = '\t'.join([str(patent_id), reference, str(match_score), str(matched_pub), author, source, str(year), title, page, str(volume), str(issue), str(no), identifier])+'\n'
            print(line)
            print("*************CANNOT WRITE THIS ENTRY TO DISK**************")
...
for year in years:
    current_path = PATH_TO_USPTO_FILES_DIR +str(year)+'/'
    
    print("Reading files ...")
    all_patents = read_all_patents(current_path)
    ...
    for key in utility_patents:
        index += 1
        for patent in utility_patents[key]:
            patent_number, patent_text = parse_V2_USPTO(patent)
